...
function App(props) {
  ...
  const [noFacing, setNoFacing] = useState("right");
  ...
  // stop shake after animation duration
  useEffect(() => {
    if (!shakeYes) return;
    const t = setTimeout(() => setShakeYes(false), 450); // match CSS duration
        ...
        <div style={{ backgroundColor: "#f3b4b4", height: "100vh", width: '100%', position: "relative", overflow: "hidden" }}>
          ...
          >
            ...
            <div style={{ display: "flex", marginLeft: 50, flexDirection: "column", alignItems: "center" }}>
              ...
              <img height={CAT_SIZE} width={CAT_SIZE} src={dirtyCat} alt="No Cat" />
              <button
